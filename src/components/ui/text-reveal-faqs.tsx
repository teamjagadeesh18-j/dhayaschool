...
export default function FAQs() {
  ...
  return (
    <section id="faqs" className="py-20 md:py-28 bg-[#fafafa] border-t border-[#e5e5e5] font-body relative overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-4 sm:px-6 md:px-8">
        <div className="grid gap-12 lg:grid-cols-12 items-start">
          
          <div className="lg:col-span-5 space-y-4">
            <span 
              className="inline-block px-3.5 py-1 rounded-xl text-xs font-extrabold uppercase tracking-widest border shadow-xs"
              style={{
            ...
            <TextEffect 
              ...
              speedReveal={1.1} 
              speedSegment={0.3} 
              className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-[#111111] font-heading tracking-tight leading-tight"
            >
            ...
            <div 
              className="p-5 rounded-xl border bg-white shadow-xs mt-6 space-y-2"
              style={{ borderColor: "#e5e5e5" }}
            >
              <h4 className="font-extrabold text-sm text-[#111111] font-heading">Need Direct Guidance?</h4>
          ...
          <div className="lg:col-span-7">
            <div 
              className="p-6 md:p-8 rounded-xl bg-white border border-[#e5e5e5]"
              style={{
              ...
              <Accordion type="single" collapsible className="space-y-3">
                {faqItems.map((item) => (
                  <AccordionItem
                    key={item.id}
                    value={item.id}
                    className="border border-slate-200/80 rounded-xl px-5 py-1 bg-slate-50/50 data-[state=open]:bg-white data-[state=open]:shadow-xs transition-all duration-200"
                  >
                    <AccordionTrigger 
                      className="cursor-pointer text-left text-sm md:text-base font-extrabold text-[#111111] hover:no-underline font-heading py-4"
                    >
                      {item.question}
                    </AccordionTrigger>
